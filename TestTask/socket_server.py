...
#     activity_table.insert("", "end", values=(username, datetime.datetime.fromtimestamp(
#         float(timestamp)).strftime("%Y-%m-%d %H:%M:%S")))
...
# start_button = ttk.Button(root, text="Запустить сервер", command=start_server)
# start_button.pack(pady=10)

# root.mainloop()

# # Закрытие сокета после выхода из главного цикла
# server_socket.close()
